...
#
# Downloads install.sh from the dist repo, then installs the sznuper release
# identified by VERSION on a Hetzner VPS and verifies the result.
#
# Usage:
...
INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/sznuper/dist/main/install.sh"
...
log "Downloading install.sh from dist repo..."
INSTALL_SCRIPT=$(mktemp)
trap 'rm -f "$INSTALL_SCRIPT"; cleanup' EXIT INT TERM
curl -fsSL "$INSTALL_SCRIPT_URL" -o "$INSTALL_SCRIPT"

log "Uploading and running install.sh with VERSION=$VERSION..."
scp -q -O "${SSH_OPTS[@]}" "$INSTALL_SCRIPT" "root@$SERVER_IP:/root/install.sh"
